...
<template>
  <div aria-hidden="true" class="blueprint" :class="variant">
    <svg viewBox="0 0 520 420" fill="none" xmlns="http://www.w3.org/2000/svg" class="bp-svg">
      ...
      <template v-else-if="variant === 'ceiling'">
        ...
        <line x1="82" y1="82" x2="358" y2="82" class="bp-mullion" style="opacity: 0.5" />
      ...
      <template v-else>
        ...
        <text x="200" y="352" text-anchor="middle" class="bp-label">Δ ≤ 0,5 мм</text>
      </template>
    </svg>
  </div>
</template>
...
.bp-node:nth-of-type(1) {
  animation-delay: 1.3s;
}
.bp-node:nth-of-type(2) {
  animation-delay: 1.4s;
}
.bp-node:nth-of-type(3) {
  animation-delay: 1.5s;
}
.bp-node:nth-of-type(4) {
...
.bp-laser {
  ...
  opacity: 0;
  animation:
    fadeIn 0.6s ease 1.1s forwards,
    dash 3s linear infinite 1.1s;
}
