  ...
  <header class="header" :class="{ scrolled }">
    <div class="container bar">
      ...
      <nav class="nav" :class="{ open: menuOpen }">
        <router-link
          v-for="p in navPages" :key="p.path" :to="p.path"
          class="nav-link" :class="{ active: route.path === p.path }"
      ...
      <div class="contacts">
        ...
        <div class="social">
          <a :href="contacts.whatsapp" class="social-btn wa" aria-label="WhatsApp">💬</a>
          <a :href="contacts.telegram" class="social-btn tg" aria-label="Telegram">✈️</a>
        </div>
      </div>
...
.nav-link:hover { color: var(--c-primary); background: rgba(27,75,102,0.06); }
.nav-link.active { color: #fff; background: var(--c-primary); }

.contacts { display: flex; align-items: center; gap: 14px; flex-shrink: 0; }
.phone {
  text-decoration: none;
  color: var(--c-ink);
  font-weight: 700;
...
.phone:hover { color: var(--c-accent); }
...
.burger span { width: 22px; height: 2px; background: var(--c-primary); border-radius: 2px; }
